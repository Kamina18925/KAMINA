...
// Endpoint para verificar sesión usando sessionToken (nueva implementación)
...
// Endpoint para verificar si la sesión sigue válida (mantener para compatibilidad)
router.get('/verify-jwt', verifySession, async (req, res) => {
  try {
...
// Endpoint de login con sessionToken
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      ...
      [user.id, sessionToken, new Date(Date.now() + 24 * 60 * 60 * 1000)] // 24 hours
    );
    
    // Return user info and session token (excluding password)
    const { password_hash, password: userPassword, ...userWithoutPassword } = user;
